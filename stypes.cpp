...
shared_ptr<ExpC> ExpC::loadStringLiteralAddr(string literal) {
    ...
    string strRegName = ralloc.getNextReg();
    string llvmCode;  // todo: check

    codeBuffer.emitGlobal(strLiteralAutoGeneratedName + "private unnamed_addr constant [" +
                          strLiteralAutoGeneratedName.length + " x i8] c\"" + literal + "\\00\"");

    llvmCode = strRegName + " = alloca ptr a ";

    return NEW(ExpC, ("STRING", STYPE2STD(string, $1)));  // TODO: edit
}
